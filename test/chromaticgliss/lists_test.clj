(ns chromaticgliss.lists-test
  (:use clojure.test
        chromaticgliss.test-core)
  (:require [chromaticgliss.models.lists :as lists]
...
(deftest add-products
  (let [user (users/create {:name "Test user" :email "me@mytest.com" :password "foo"})
    ...
    (testing "Adds product to existing list"
      (let [modified-list (lists/add-product my-list (:id pintos))]
        (is (= (seq [pintos]) (:products modified-list)))))

    (testing "Creates new list with products"
      (let [listdata (lists/create {:user_id (:id user)
                                    :title "Most interesting"
                                    :products [pintos]})]
        (is (= (seq [pintos]) (:products listdata)))
        (is (= (seq [pintos]) (:products (lists/find-by-id (:id listdata)))))))
    (testing "Creates products added with an update"
      (let [listdata (lists/create {:user_id (:id user)
        ...
        (is (= (seq [pintos coffee]) (:products updated)))
        (is (= (seq [pintos coffee]) (:products (lists/find-by-id (:id listdata)))))))))
  ...
  (let [user (users/create {:name "Test user" :email "me@mytest.com" :password "foo"})
    ...
    (testing "Removes a product from a list"
      (let [modified-list (lists/remove-product my-list (:id kidneys))]
        (is (= (seq [limas]) (:products modified-list)))))
    (testing "Removes products absent from an update"
      (let [coffee (products/create {:title "Coffee Beans"
        ...
        (is (= (seq [coffee]) (:products updated)))
        (is (= (seq [coffee]) (:products (lists/find-by-id (:id listdata)))))))))
